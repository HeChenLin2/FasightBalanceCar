...
struct dmp_data myDmpData = {0,0,0};				//dmp解算后的姿态角度
struct accelerated_speed acc = {0,0,0};			//加速度计原始数据
struct gyroscope gyrodata = {0,0,0}; 						//陀螺仪原始数据
...
u8 mpu_dmp_get_data(float *pitch,float *roll,float *yaw)
{
	...
	**/
	if(sensors&INV_WXYZ_QUAT) 
	{
		...
		*roll  = (-1) * atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2* q2 + 1)* 57.3;	// roll
